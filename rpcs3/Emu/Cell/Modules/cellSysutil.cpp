...
s32 CXmlAddGetStringOffset(vm::ptr<cxml_document> cxmlDoc, vm::cptr<char> name, u32 nameLen)
{
    ...
    if (cxmlDoc->stringtable_capacity < (cxmlDoc->header.stringtable_size + nameLen + 1))
    {
        ...
        _ZN16sysutil_cxmlutil11FixedMemory8AllocateEN4cxml14AllocationTypeEPvS3_jPS3_Pj(AllocationType_Alloc_StringTable, cxmlDoc->allocator_userdata, cxmlDoc->stringtable, reqSize, addr, size);
        if (addr == vm::null)
...
{
    cellSysutil.error("_ZN16sysutil_cxmlutil12PacketWriterC1EiiRN4cxml8DocumentE(packetInfo=*0x%x, slot=%d, evnt=0x%x, cxmlDoc=*0x%x)", packetInfo, slot, evnt, cxmlDoc);
...
s32 _ZN16sysutil_cxmlutil12PacketWriter5WriteEPKvjPv(ppu_thread& ppu, u32 addr, u32 size, vm::ptr<SysutilPacketInfo> packetInfo) {
    cellSysutil.warning("_ZN16sysutil_cxmlutil12PacketWriter5WriteEPKvjPv(data=*0x%x, size=0x%x, packetInfo=*0x%x)", addr, size, packetInfo);
...
// sweet, this funcptr callback suffers the same reloc issue as the allocator function which ive mentioned above in the file, the good news is that internally, sysutil seems to just use 7FC8F72C ( _ZN16sysutil_cxmlutil12PacketWriter5WriteEPKvjPv), so lets just try that for now
